...
class PanelManager {
		...
		return new Promise((resolve, reject) => {
			...
			if (!$dom.hasClass('__init__') && $dom.hasClass('_active') && isShow) {
				...
				if (win.extends && params.data) {
					!!win.extends[isShow ? 'show' : 'hide'] &&
						win.extends[isShow ? 'show' : 'hide'].call(win, params.data || null);
				}
				return false;
			}
	...
	batchTogglePanelShow(panels, isShow, params) {
		...
		if (!this.cache.lock) {
			...
			panels.forEach((v) => {
				let layer = this.layers[v];
				result.push(
					this.toggleComponentShow(v, isShow, layer, {
						cache: opt.cache,
						data: opt.data,
						hide: opt.hide,
					})
	...
	emit(key, data) {
		...
		for (let i = 0, len = this.events.length; i < len; i++) {
			if (
				this.events[i].key == key &&
				Object.prototype.toString.call(this.events[i].fn) == '[object Function]'
			) {
